    ...
    if argument == 'stream':
        ...
        await client.change_presence(
            ...
            activity=discord.Activity(
                ...
                name = "BLOOD-SELF",
            assets = {
                'large_image_url' : 1120695317944336424,
                'large_text':'Nuke them all',
                'small_image_url': 1120684679503294474
            },
            url = 'https://twitch.tv/404',
            timestamps = {
                'start': 1645701071
...
async def cinfo(ctx):
    ...
    await ctx.send(f"""```"Total cores:" {psutil.cpu_count(logical=True)}```""")

    await ctx.send(f"""```CPU Frequency:```""")
    cpufreq = psutil.cpu_freq()
    await ctx.send(f"""```Max Frequency: {cpufreq.max:.2f}Mhz```""")
    await ctx.send(f"""```Current Frequency: {cpufreq.current:.2f}Mhz```""")
    
    await ctx.send(f"""```CPU Usage:```""")

    await ctx.send(f"""```Total CPU Usage: {psutil.cpu_percent()}%```""")

@client.command()
async def version(ctx):
    uname = platform.uname()
    await ctx.send(f"""```
Selfbot version is {Selfbot.__version__} 

DevMode - {Selfbot.__devmode__}                   
